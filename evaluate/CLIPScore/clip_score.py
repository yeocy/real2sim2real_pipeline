...
def main():
    ...
    clip_score_result = {}
    for i, feat in enumerate(image_feats):
        feat = np.expand_dims(feat, axis=0)
        _, per_instance_image_text, candidate_feats = get_clip_score(
            model, feat, candidates, device)
        clip_score_result[Path(image_paths[i]).stem] = per_instance_image_text.tolist()
    
    with open('Clip_Scores.json', 'w') as f:
        json.dump(clip_score_result, f, indent=2)
