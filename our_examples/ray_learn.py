...
def train(env_name, algo, timesteps_total=2000000, save_dir='./trained_models/', load_policy_path='', seed=0, 
    ...
    if not ray.is_initialized():
        ...
        # ray.init(num_gpus=2, ignore_reinit_error=True, log_to_driver=False)
    ...
    while timesteps < timesteps_total:
        ...
        if timesteps > eval_time * eval_interval:
            ...
            eval_model_save_path = os.path.join(save_dir, "eval_{}".format(eval_time))
            if not os.path.exists(state_save_path):
                os.makedirs(state_save_path)
            eval_model_path = agent.save(eval_model_save_path)
            while not done:
...
def make_env(config, render=False):
    ...
    print(f"### make_env | config: {config}")
    ...
    #         camera_position=config['camera_position'] if 'camera_position' in config.keys() else None,
    #         camera_orientation=config['camera_orientation'] if 'camera_orientation' in config.keys() else None,
    #         use_gpt_spatial_relationship=config['use_gpt_spatial_relationship'],
    #         trajectory_save_path=config['reward_trajectory_save_path'] if 'reward_trajectory_save_path' in config.keys() else "",
    #         randomize_obj_name=config['randomize_obj_name'] if 'randomize_obj_name' in config.keys() else None,
    #         func_after_reset=config['func_after_reset'] if 'func_after_reset' in config.keys() else None,
    #     )
    ...
    kwargs = {
        ...
        'task_name': task_name,
        'restore_state_file': last_restore_state_file,
        'render': render,
        'action_space': action_space,
        'randomize': config['randomize'],
        'obj_id': config['obj_id'],
        'robot_name': config['robot_name'],
        'robot_scale': config['robot_scale'],
        'table_id': config['table_id'],
        'use_gpt_spatial_relationship': config['use_gpt_spatial_relationship']
    }

    # 선택적 파라미터는 config에 있을 때만 추가
    if 'camera_position' in config:
        kwargs['camera_position'] = config['camera_position']
    if 'camera_orientation' in config:
        kwargs['camera_orientation'] = config['camera_orientation']
    if 'reward_trajectory_save_path' in config:
        kwargs['trajectory_save_path'] = config['reward_trajectory_save_path']
    if 'randomize_obj_name' in config:
        kwargs['randomize_obj_name'] = config['randomize_obj_name']
    if 'func_after_reset' in config:
        kwargs['func_after_reset'] = config['func_after_reset']
    if 'observe_only_target_obj' in config:
        kwargs['observe_only_target_obj'] = config['observe_only_target_obj']
    if 'observe_only_target_names' in config:
        kwargs['observe_only_target_names'] = config['observe_only_target_names']

    env, safe_config = build_up_env(**kwargs)
...
def run_RL(task_config_path, solution_path, task_name, last_restore_state_file, save_path, 
           ...
           render=False, randomize=False, use_bard=True, obj_id=0, table_id=None, robot_name=None, robot_scale=1,
           camera_position=[0, 0, 0.3], camera_orientation=[0, -30, -30],
           randomize_obj_name=None, func_after_reset=None, observe_only_target_obj=False, observe_only_target_names=None,
    ...
    env_config = {
        ...
        "randomize_obj_name": randomize_obj_name,
        "func_after_reset": func_after_reset,
        "observe_only_target_obj": observe_only_target_obj,
        "observe_only_target_names": observe_only_target_names,
    ...
    print(f"### run_RL | env_config: {env_config}")
