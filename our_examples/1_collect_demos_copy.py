...
# VISUALIZE_SKILL = True                                         # Whether to visualize skill during demo collection or not
# XYZ_RANDOMIZATION = np.array([0.03, 0.03, 0.07])                # (x,y,z) randomization to apply to target object
XYZ_RANDOMIZATION = np.array([0.01, 0.01, 0.01])                # (x,y,z) randomization to apply to target object
# Z_ROT_RANDOMIZATION = np.pi / 10                                # z-rotation randomization to apply to target object
Z_ROT_RANDOMIZATION = 0                                # z-rotation randomization to apply to target object
...
DEFAULT_DIST_FROM_HANDLE = np.array([0.5, 0.1, -0.8])           #! default offset from the robot base to the target object's handle  (OpenCabinetWrapper + FrankaPanda)
# DEFAULT_DIST_FROM_HANDLE = np.array([0.7, 0.3, -1.2])              # default offset from the robot base to the target object's handle  ("Pick up the bowl in the drawer"; OpenCabinetPickBowlWrapper + FrankaMounted)
# DEFAULT_DIST_FROM_HANDLE = np.array([0.75, 0.3, -0.3])           # default offset from the robot base to the target object's handle  ("Pick up the bowl in the drawer"; OpenCabinetPickBowlWrapper + FrankaPanda)
# DEFAULT_DIST_FROM_HANDLE = np.array([0.32, 0.0, -1.2])           # default offset from the robot base to the target object's handle  ("Pick up the can in the sink"; PickCanInSinkWrapper) + FrankaMounted
# DEFAULT_DIST_FROM_HANDLE = np.array([0.37, 0.0, -0.3])           # default offset from the robot base to the target object's handle  ("Pick up the can in the sink"; PickCanInSinkWrapper) + FrankaPanda
# BBOX_TRAIN_RANDOMIZATION = np.array([0.05, 0.05, 0.05]) * 5.0   # Relative % randomization scaling of target objects' bboxes
BBOX_TRAIN_RANDOMIZATION = np.array([0.05, 0.05, 0.0]) * 2.0   # Relative % randomization scaling of target objects' bboxes
# BBOX_TRAIN_RANDOMIZATION = np.array([0, 0, 0]) * 5.0   # Relative % randomization scaling of target objects' bboxes
STEP_DIVISOR = 5                                                # Minimum physics steps to take (@60Hz) per action
STEP_BY_STEP = False                                            # Whether to collect demos step-by-step or not (if True, will wait for user input before taking each step)
# STEP_BY_STEP = True
...
def main(args):
    ...
    robot_params = {
        ...
        "model_name": "FrankaPanda",      # Only currently works for FrankaPanda, FrankaMounted models
        "robot_name": "robot0",
        "reset_qpos": th.tensor([-0.0027, -1.3000, -0.0012, -2.0000, -0.0082, 2.1875, 0.8032, 0.0400, 0.0400]),
        # "eef_z_offset": 0.180,  #! FrankaMounted
        "eef_z_offset": 0.093,  # FrankaPanda
        "open_qpos": None,              # If specified, joint values defining an open state for the robot gripper
        # "root_link": "panda_base",  #! FrankaMounted
        "root_link": "panda_link0",  # FrankaPanda
        # "vis_local_position": th.tensor([-0.26549, -0.30288, 1.0 + 0.861]),
        # "vis_local_orientation": th.tensor([0.36165891, -0.24745751, -0.50752921, 0.74187715]),  # quaternion (xyzw)
        "vis_local_position": th.tensor([-0.23505797, -0.67757382,  1.29959649]),  #! OpenCabinetWrapper + FrankaPanda
        "vis_local_orientation": th.tensor([ 0.51644084, -0.1885153 , -0.28670213,  0.78457168]),  # quaternion (xyzw)
        # "vis_local_position": th.tensor([ 0.1038095,  -0.89195126,  2.20825328]),  # OpenCabinetPickBowlWrapper + FrankaMounted
        # "vis_local_orientation": th.tensor([ 0.45620654, -0.07441919, -0.15386214,  0.87330626]),
        # "vis_local_position": th.tensor([-0.37181169, -1.01076433,  1.39826467]),  # OpenCabinetPickBowlWrapper + FrankaPanda
        # "vis_local_orientation": th.tensor([ 0.46005487, -0.14274805, -0.26941449,  0.83389948]),
        # "vis_local_position": th.tensor([-0.3210771,   0.76956769,  2.85664876]),  # PickCanInSinkWrapper + FrankaMounted
        # "vis_local_orientation": th.tensor([ 0.16432553, -0.28865501, -0.82101074,  0.46434553]),
        # "vis_local_position": th.tensor([-0.35037015,  0.49793935,  1.51506216]),  # PickCanInSinkWrapper + FrnakaPanda
        # "vis_local_orientation": th.tensor([ 0.16432553, -0.28865501, -0.82101074,  0.46434553]),
    }
    ...
    cfg["wrapper"] = {
        ...
        "handle_dist": 0.005,
        "approach_dist": 0.3,  # Approach distance to the target handle
    ...
    # cfg["wrapper"] = {
    #     "type": "OpenCabinetPickBowlWrapper",
    #     "eef_z_offset": robot_params["eef_z_offset"],
    #     "cab_categories": cousin_category_names,
    #     "cab_models": cousin_model_names,
    #     "cab_links": cousin_link_names,
    ...
    #     "visualize_skill": VISUALIZE_SKILL,
    #     "custom_bddl": None,
    #     "task_activity_name": "pick_can",
    #     "scene_info": scene_info,
    #     "scene_target_parent_obj_name": args.target_parent_obj,
    #     "scene_target_child_obj_name": args.target_child_obj,
    # }
    # cfg["wrapper"] = {
    #     "type": "TestPrimitiveActionsWrapper",
    #     "eef_z_offset": robot_params["eef_z_offset"],
    #     "cab_categories": cousin_category_names,
    #     "cab_models": cousin_model_names,
    #     "cab_links": cousin_link_names,
    ...
    #     "visualize_skill": VISUALIZE_SKILL,
    #     "custom_bddl": None,
    #     "task_activity_name": "test_primitive_actions",
    #     "scene_info": scene_info,
    #     "scene_target_parent_obj_name": args.target_parent_obj,
    #     "scene_target_child_obj_name": args.target_child_obj,
    # }
    ...
    env = EnvOmniGibson(
        ...
        obs_modalities=[
            ...
            f"{robot_params['robot_name']}::robot0:wrist_eye:Camera:0::point_cloud",
        ],
        combine_pc=True,
        include_segment_strs=["cabinet", "bowl", "panda_hand", "panda_link", "panda_leftfinger", "panda_rightfinger"],  #! OpenCabinetPickBowlWrapper
        # include_segment_strs=["sink", "can", "panda_hand", "panda_link", "panda_leftfinger", "panda_rightfinger"],  # PickCanInSinkWrapper
        # include_eef_pc=True,
        include_eef_pc=False,  # EEF 전체를 pointcloud에 포함하지 않고 실제로 보이는 부분만 포함
        embed_eef_pc=True,
        max_pc=2048,
        og_config=cfg,
        ...
        # external_cam_depth_threshold=10,  # original value
        external_cam_depth_threshold=10,  # ! OpenCabinetWrapper
        # external_cam_depth_threshold=2.5,  # OpenCabinetPickBowlWrapper
        # external_cam_depth_threshold=3.0,  # PickCanInSinkWrapper
    ...
    ic("Wrap with skill data collection")
    ...
    # arm_controller = env.env.env.robots[0].controllers[f"arm_0"]
    # for attr in dir(arm_controller):
    #     if not attr.startswith("__"):
    #         try:
    #             print(f"{attr}: {getattr(arm_controller, attr)}")
    #         except Exception as e:
    #             print(f"{attr}: <{type(e).__name__}>")
    # print("--------------------------------------------------")

    ic(f"Collecting {args.n_demos_per_model} demos per model, excluding eval model {cousin_model_names[eval_cousin_id]}")
    for i, model in enumerate(cousin_model_names):
        ...
        while n_cab_successes < args.n_demos_per_model:
            ...
            env.reset()
            env.collect_demo(step_by_step=STEP_BY_STEP)

            # # (special case) 무조건 첫 trajectory 로깅 후 종료
            # env.flush_current_traj_no_check()
            # break

            ic(env.env.is_success())
            if env.env.is_success()["task"]:
                ...
                n_cab_fails += 1
                og.log.warning(f"Collected model {model} failed demo n: {n_cab_fails}")
            video_count += 1
            n_trials += 1
            og.log.warning(f"Model {model} trial n: {n_trials}, successes: {n_cab_successes}, fails: {n_cab_fails}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target_obj", type=str, required=False,
                        help="Name of the object to articulate")
    parser.add_argument("--target_parent_obj", type=str, required=False,
                        help="Name of the object to articulate")
    parser.add_argument("--target_child_obj", type=str, required=False,
                        help="Name of the object to articulate")
