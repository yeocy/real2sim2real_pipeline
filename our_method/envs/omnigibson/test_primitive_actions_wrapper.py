...
from our_method.skills.grab_object import GrabObjectSkill
from our_method.skills.rotate_gripper import RotateGripperSkill
from our_method.skills.translate_gripper import TranslateGripperSkill
...
class TestPrimitiveActionsWrapper(SkillWrapper):
    """
    ...
    def __init__(
            ...
            task_obj_bddl_name="sink.n.01_1",
            task_child_obj_bddl_name="can__of__tomato_paste.n.01_1",
            custom_bddl=None,
            max_steps=500,
            skill_kwargs=None,
        ...
        self.task_child_obj_bddl_name = task_child_obj_bddl_name
        ...
        for i, (cab_category, cab_model, cab_link, cab_bbox, randomize_tex) in \
            ...
            with og.sim.stopped():
                ...
                if self.scene_info is not None:
                    ...
                    ic(target_child_obj_bbox_pos, target_child_obj_bbox_quat)
    ...
    def reset(self):
        ...
        # ic(self._current_idx)
        self.cabs[self._current_idx].visible = False
        self.target_objs[self._current_idx].visible = False
        # for link_name, link_info in self.cabs[self._current_idx].links.items():
        #     ic(link_name, link_info.visible)
        # for link_name, link_info in self.target_objs[self._current_idx].links.items():
        #     ic(link_name, link_info.visible)
        ...
        for link_name, link_info in self.cabs[self._current_idx].links.items():
            # ic(link_name, link_info.visible)
            link_info.visible = True  # Make sure all links are visible
            print(f"link_name, link_info.visible: {link_name, link_info.visible}")
        # for link_name, link_info in self.target_objs[self._current_idx].links.items():
        #     ic(link_name, link_info.visible)
        ...
        self.env.task.object_scope[self.task_child_obj_bddl_name].wrapped_obj = self.target_objs[self._current_idx]
        ...
        if self.target_parent_obj_position is not None and self.target_parent_obj_orientation is not None and self.scene_info is None:
            ...
            allowed_y_min = cab_aabb_min[1] + target_y_width + padding*2
            allowed_y_max = (cab_aabb_max[1] + cab_aabb_min[1]) / 2.0
            ...
            allowed_x_min = cab_aabb_min[0] + target_x_width + padding*4
            allowed_x_max = cab_aabb_max[0] - target_x_width - padding*4
        ...
        max_steps = 1000  # Safety limit to prevent infinite loop
        step_count = 0
        while not target_obj.states[Touching].get_value(cab_obj) and step_count < max_steps:
            og.sim.load_state(old_state)
        ...
        if step_count >= max_steps:
            print("Warning: Reached maximum step count while trying to place object on cabinet")
        else:
            print(f"Object placed on cabinet after {step_count} steps.")
        ...
        for _obj in self.scene.objects:
            if ToggledOn in _obj.states:
                ic(_obj.name, _obj.states[ToggledOn].get_value(), _obj.states[ToggledOn].link, _obj.states[ToggledOn].link.visible)
                lnk_ks = [lnk_k for lnk_k in _obj.links.keys()]
                ic(lnk_ks)
                lnk_vs = [lnk_itm.visible for lnk_itm in _obj.links.values()]
                ic(lnk_vs)
    ...
    def default_bddl(self):
        ...
        """
        return """
        (define (problem pick_can-0)
            (:domain omnigibson)

            (:objects
                sink.n.01_1 - sink.n.01
                can__of__tomato_paste.n.01_1 - can__of__tomato_paste.n.01
                agent.n.01_1 - agent.n.01
            )

            (:init
                (inroom sink.n.01_1 none)
                (ontop can__of__tomato_paste.n.01_1 sink.n.01_1)
            )

            (:goal
                (touching can__of__tomato_paste.n.01_1 agent.n.01_1)
            )
        )
        """
