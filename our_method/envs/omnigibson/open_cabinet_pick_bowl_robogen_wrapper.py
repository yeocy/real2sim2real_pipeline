...
# from deps.OmniGibson.omnigibson.objects.usd_object import USDObject
import torch as th
from our_method.envs.omnigibson.skill_wrapper import SkillWrapper
from our_method.skills.open_and_grab_bowl import OpenAndGrabBowlSkill
...
class OpenCabinetPickBowlRobogenWrapper(SkillWrapper):
    """
    ...
    def __init__(
            ...
            task_child_obj_bddl_name="bowl.n.01_1",
            custom_bddl=None,
            max_steps=500,
            skill_kwargs=None,
        ...
        if self.scene_info is not None:
            ...
            with og.sim.stopped():
                for obj_name, obj_info in self.scene_info["objects"].items():
                    ...
                    # print(self.env.scene.object_registry("in_rooms", None, default_val=[]))
                    # exit()
        ...
        for i, (cab_category, cab_model, cab_link, cab_bbox, randomize_tex) in \
            ...
            with og.sim.stopped():
                ...
                #     fixed_base=False,
                # )
                cab = USDObject(name=f"cabinet{i}",
                        # visual_only=obj_name =  = self.scene_target_parent_obj_name or self.non_target_objs_visual_only,
                        # visual_only= not (obj_name == self.scene_target_child_obj_name or obj_name == self.scene_target_parent_obj_name),
                        usd_path=self.scene_info["objects"][self.scene_target_parent_obj_name]["usd_path"],
                        bounding_box=cab_bbox,
                        fixed_base=True
                    )
                
                target_obj = USDObject(
                        name=f"bowl{i}",
                        # visual_only=obj_name == self.scene_target_parent_obj_name or self.non_target_objs_visual_only,
                        # visual_only= not (obj_name == self.scene_target_child_obj_name or obj_name == self.scene_target_parent_obj_name),
                        usd_path=self.scene_info["objects"][self.scene_target_child_obj_name]["usd_path"],
                        bounding_box=self.scene_info["objects"][self.scene_target_child_obj_name]["bbox_extent"],
                        fixed_base=False
                    )
        ...
        print(f"[OpenCabinetPickBowlRobogenWrapper] task_config: {task_config}")
    ...
    def reset(self):
        ...
        if self.target_parent_obj_position is not None and self.target_parent_obj_orientation is not None and self.scene_info is None:
            ...
            allowed_y_max = cab_aabb_max[1] * 1/3 + cab_aabb_min[1] * 2/3
