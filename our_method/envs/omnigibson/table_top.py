...
from our_method.skills.pick_and_place_kinova import PickAndPlaceSkill
...
class TableTopWrapper(SkillWrapper):
    """
    ...
    def __init__(
            ...
            with og.sim.stopped():
                for obj_name, obj_info in self.scene_info["objects"].items():
                    ...
                    obj = DatasetObject(
                        ...
                        fixed_base=True if obj_name == "table_0" else not obj_info["mount"]["floor"],
                    ...
                    # (추가) pencil_case일 때 z축 기준 90도 회전
                    if "pencil_case" in obj_name:
                        print(obj_pos)
                        z_rot_90 = OT.euler2quat(th.tensor([0, 0, 0], dtype=th.float))
                        obj_quat = OT.quat_multiply(z_rot_90, th.tensor(obj_quat, dtype=th.float))
                        obj_pos = obj_pos- th.tensor([-0.2, 0.0, 00])

                    if "box" in obj_name:
                        obj_pos = obj_pos- th.tensor([-0.1, 0.15, 00])


                    # (추가) pencil_case나 box일 때 z축을 0.005 올려줌
                    if "pencil_case" in obj_name or "box" in obj_name:
                        obj_pos[2] += 0.025  # z축을 0.005만큼 올려줌
        ...
        # print(cab_categories, cab_models, cab_links)
        # print(self._default_cab_bboxs)
        # exit()

        for i, (cab_category, cab_model, cab_bbox, randomize_tex) in \
                enumerate(zip(cab_categories, cab_models, self._default_cab_bboxs, self._randomize_textures)):
            with og.sim.stopped():
            ...
            # table_obj = self.env.scene.object_registry("name", "table_0")
            # # 💥 Coffee box (target_obj) 전체 링크에 적용
            # for link in table_obj.links.values():
            #     for msh in link.collision_meshes.values():
            #         msh.apply_physics_material(gripper_mat)
        ...
        # task_config["activity_name"] = "pick_and_place"
    ...
    def default_bddl(self):
        ...
        """
        return """
        (define (problem pick_pencil_case)
            (:domain omnigibson)

            (:objects
                pencil_case.n.01_1 - pencil_case.n.01
                agent.n.01_1 - agent.n.01
            )

            (:init
                (inroom pencil_case.n.01_1 none)
                (at pencil_case.n.01_1 initial_position)  ; pencil_case가 특정 위치에 있다고 가정
            )

            (:goal
                (at pencil_case.n.01_1 target_position)  ; 목표는 pencil_case가 target_position에 있다는 것
            )
        )
        """
