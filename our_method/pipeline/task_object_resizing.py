...
##########################################
        ...
        if use_distractor_noise:
            feature_matching_distractor_path = os.path.join(os.path.dirname(task_feature_matching_path), "distactor")

            folder_list = [
                name for name in os.listdir(feature_matching_distractor_path)
                if os.path.isdir(os.path.join(feature_matching_distractor_path, name))
            ]
            for parent_object_name in folder_list:
                parent_object_path = os.path.join(feature_matching_distractor_path, parent_object_name)

                with open(os.path.join(parent_object_path, "distractor_output_info.json"), "r") as f:
                    distractor_object_retrieval_output_info = json.load(f)

                distractor_json_list = []
                distractor_dir = os.path.join(save_dir, "distractor", parent_object_name)
                Path(distractor_dir).mkdir(parents=True, exist_ok=True)
                for scenario_obj_num_json_path in distractor_object_retrieval_output_info:
                    with open(scenario_obj_num_json_path, "r") as f:
                        distractor_obj_output_info = json.load(f)

                    if resizing:
                        # Get current object size
                        obj_size_info = {}
                        for obj_name, obj_info in distractor_obj_output_info["objects"].items():
                            # 기존 obj_name 사용 전에 안전하게 변환
                            obj_name_ = obj_name.replace(" ", "_")
                            # Add the object in the scene
                            obj = DatasetObject(
                                    name=obj_name_,
                    ...
                    distractor_obj_resize_info = copy.deepcopy(distractor_obj_output_info)
                    ...
                    with open(distractor_object_resizing_path, "w+") as f:
                        # json.dump(task_obj_resize_info, f, indent=4)
                        write_json_like(distractor_obj_resize_info, f, indent=0)
                


                with open(f"{distractor_dir}/distractor_object_resizing_path.json", "w+") as f:
                # json.dump(task_extraction_output_info, f, indent=4, 
                #           cls=OneLineListEncoder)
