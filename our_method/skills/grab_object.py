...
class GrabObjectSkill(ManipulationSkill):
    """
    ...
    def compute_current_subtrajectory(
        ...
        if step == GrabObjectStep.TARGET_READY:
            ...
            #  Gripper 기준 위치 이동
            joint_to_grasp_pos = current_global_eef_pos.clone()
            # joint_to_grasp_pos[0] -= 0.1
            joint_to_grasp_pos[1] += 0.1
            joint_to_grasp_pos[2] = current_global_eef_pos[2] - gripper_length + z_offset
            # # target_child의 중앙으로 이동
            # joint_to_grasp_pos = target_child_pos.clone()
            # joint_to_grasp_pos[2] = current_global_eef_pos[2] - gripper_length + z_offset
            
            grasp = False
        elif step == GrabObjectStep.TARGET_APPROACH:
            self.target_step = True
            ...
            # target_child의 중앙 위로 이동
            joint_to_grasp_pos = target_child_pos.clone()
            joint_to_grasp_pos[1] = target_child_pos[1] + y_offset
            joint_to_grasp_pos[2] = target_child_pos[2] + z_offset

            grasp = False
        # () Converge to Target Object
        elif step == GrabObjectStep.TARGET_CONVERGE:
            self.target_step = True
            n_steps = n_converge_steps * 2
            ic(n_converge_steps, n_steps)
            # # joint_to_grasp_pos = self._joint_to_handle_pos
        ...
        elif step == GrabObjectStep.TARGET_GRASP:
            self.target_step = True
        ...
        elif step == GrabObjectStep.TARGET_UP:
        ...
        # elif step == OpenAndGrabBowlStep.TARGET_PLACE:
        #     self.target_step = True
        #     self.target_approach = 0.0
        #     n_steps = n_approach_steps*2

        #     initial_global_eef_pos = self._initial_eef_pos + self._robot.get_position_orientation()[0]
        #     joint_to_grasp_pos = initial_global_eef_pos + th.tensor([0.1, 0.1, 0.5], dtype=th.float)
        #     # joint_to_grasp_pos = self._joint_to_approach_target_pos
        #     grasp = True
        # () Release grasp
        # elif step == OpenAndGrabBowlStep.TARGET_UNGRASP:
        #     self.target_step = True
        #     n_steps = n_grasp_steps
        #     no_op = True
        #     grasp = False
        # elif step == OpenAndGrabBowlStep.TARGET_RELEASE:
        #     self.target_step = True
