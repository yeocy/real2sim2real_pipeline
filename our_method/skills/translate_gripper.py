...
class TranslateGripperStep(IntEnum):
    Translate_Xp = auto()
    Translate_Xm = auto()
    Translate_Yp = auto()
    Translate_Ym = auto()
...
class TranslateGripperSkill(ManipulationSkill):
    """
    ...
    def compute_current_subtrajectory(
        ...
        if step == TranslateGripperStep.Translate_Xp:
            n_steps = 30
            ic(n_steps)
            ...
            delta_translation = th.tensor([0.1, 0, 0], dtype=th.float)  # (robot frame 기준)
            target_pos += OT.quat2mat(current_global_eef_orientation) @ delta_translation  # world frame 기준으로 이동

            # Rotation
            target_mat = OT.quat2mat(current_global_eef_orientation)
            ic(target_mat, OT.quat2axisangle(OT.mat2quat(target_mat)))

            grasp = False

        elif step == TranslateGripperStep.Translate_Xm:
            n_steps = 30
            ic(n_steps)
            gripper_length = self._eef_z_offset
        ...
        # () Converge to Target Object
        elif step == TranslateGripperStep.Translate_Yp:
            n_steps = 30
            ic(n_steps)
            gripper_length = self._eef_z_offset
            ...
            delta_translation = th.tensor([0, 0.1, 0], dtype=th.float)  # (robot frame 기준)
            target_pos += OT.quat2mat(current_global_eef_orientation) @ delta_translation  # world frame 기준으로 이동
        ...
        elif step == TranslateGripperStep.Translate_Ym:
            n_steps = 30
            ic(n_steps)
            gripper_length = self._eef_z_offset
        ...
        # () UP the Target Object
        elif step == TranslateGripperStep.Translate_Zp:
            n_steps = 30
            ic(n_steps)
            gripper_length = self._eef_z_offset
            ...
            delta_translation = th.tensor([0, 0, 0.1], dtype=th.float)  # (robot frame 기준)
            target_pos += OT.quat2mat(current_global_eef_orientation) @ delta_translation  # world frame 기준으로 이동
        ...
        elif step == TranslateGripperStep.Translate_Zm:
            n_steps = 30
            ic(n_steps)
            gripper_length = self._eef_z_offset
        ...
        #####################
        if step_by_step:
            if step in TranslateGripperStep:
