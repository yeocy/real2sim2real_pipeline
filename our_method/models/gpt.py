...
class GPT:
    ...
    def __init__(
        ...
        Args:
            ...
            version (str): GPT version to use. Valid options are: {4o, 4o-mini, 4v, 5.1}
        ...
        self.token_print = token_print
        
        # Initialize OpenAI client
        self.client = OpenAI(api_key=api_key)
    
    def calculate_cost(self, prompt_tokens, completion_tokens, cached_tokens=0):
        """
        Calculate the cost of API call based on token usage
        
        Args:
            prompt_tokens (int): Number of input tokens (non-cached)
            completion_tokens (int): Number of output tokens
            cached_tokens (int): Number of cached input tokens (50% discount)
            
        Returns:
            tuple: (input_cost, cached_cost, output_cost, total_cost) or None if model not found
        """
        # 모델별 비용 (per 1M tokens) - 2025년 11월 기준
        ...
        cost_per_model = {
            ...
            "gpt-5-pro": {"input": 15.00, "cached": None, "output": 120.00},
            ...
            # Audio models
            "gpt-audio": {"input": 2.50, "cached": None, "output": 10.00},
            "gpt-audio-mini": {"input": 0.60, "cached": None, "output": 2.40},
            ...
            "o3-pro": {"input": 20.00, "cached": None, "output": 80.00},
            ...
            # Image models
            "gpt-image-1": {"input": 5.00, "cached": 1.25, "output": None},
            "gpt-image-1-mini": {"input": 2.00, "cached": 0.20, "output": None},
            
            # Legacy models
            "gpt-4-vision-preview": {"input": 10.00, "cached": None, "output": 30.00},
        }
        
        model_name = self.VERSIONS[self.version]
        if model_name in cost_per_model:
            pricing = cost_per_model[model_name]
    ...
    def __call__(self, payload, verbose=False):
        ...
        while attempts < self.max_retries:
            try:
                if verbose:
                    log.info(f"Querying GPT-{self.version} API...")
                ...
                # Log query contents
                log_path = self.log_query_contents(
                    request_msg=payload, 
                    request_img_path="", 
                    response=response, 
                    log_dir_tail=self.log_dir_tail
                )
                
                if verbose:
                    log.info(f"Finished querying GPT-{self.version}.")
                    log.info(f"GPT Query & Response Saved at {log_path}.")
                    ...
                    if self.token_print:
                        ...
                        if hasattr(response, 'usage') and response.usage:
                            ...
                            log.info(f"📊 Tokens: {prompt_tokens} prompt, {cached_tokens} cached, "
                                f"{completion_tokens} completion, {total_tokens} total")
                            
                            cost_info = self.calculate_cost(prompt_tokens, completion_tokens, cached_tokens)
                            if cost_info:
                                input_cost, cached_cost, output_cost, total_cost = cost_info
                                log.info(f"💰 Cost: ${input_cost:.6f} input, ${cached_cost:.6f} cached, "
                ...
                attempts += 1
                log.error(f"Error querying GPT-{self.version} API: {e}")
                if attempts < self.max_retries:
                    log.warning("Retrying in 5 seconds...")
                    time.sleep(5)
                else:
                    log.error(f"Failed to query GPT-{self.version} API after {self.max_retries} attempts.")
                    return None
    ...
    def log_query_contents(self, request_msg, request_img_path, response, log_dir_base="logs", log_dir_tail=""):
        ...
        if request_img_path and os.path.exists(request_img_path):
            ...
            log.info(f"이미지 저장 완료 ({destination})")
    ...
    def payload_get_object_caption(self, img_path):
        """
        Generates custom prompt payload for object captioning (Responses API)
        ...
        instructions = (
            ...
            "### Task ###\n"
            "Given an image, output a Python-style list of all visible objects.\n"
            "Each object should be described clearly and specifically enough to distinguish it from "
            "other *different categories* of objects in the scene.\n\n"

            "### Rules ###\n"
            "1. Use names that correctly reflect the object's category.\n"
            "   - It's fine to use general names like 'mug', 'pan', 'plate', or 'bowl'.\n"
            "   - But do NOT collapse different categories into one term.\n"
            "     (e.g., do NOT label 'orange juice carton', 'sauce bottle', and 'water bottle' all as 'bottle').\n\n"

            "2. Add ONLY the details necessary to distinguish object types.\n"
            "   Do NOT include minor appearance details (e.g., cap color, shape variation, small texture differences).\n"
            "   Example: use 'plastic bottle', not 'plastic bottle with white cap'.\n\n"

            "3. List each object instance separately if they differ in type.\n"
            "4. Treat each object as one unit (e.g., 'cabinet', not 'door').\n"
            "5. Count only visible openable doors/drawers (cabinet(3 doors & 2 drawers)).\n"
            "6. Ignore walls, floors, windows, ceiling items; include wall-mounted objects.\n"
            "7. Do not hallucinate objects not present in the image.\n"
            "8. Output only the list, no explanation.\n\n"

            "### Example Outputs ###\n"
            '["mug", "stainless frying pan", "wooden cabinet(2 doors)"]\n'
            '["orange juice carton", "plastic bottle", "cutting board"]\n'
            '["laptop", "mouse", "office chair"]\n'
        )
        ...
        input_messages = [
            {
                ...
                "content": [
                    {
                        "type": "input_text",
                        "text": "Now please provide a list of all objects ([object1, object2, ...]) "
    ...
    def payload_filter_wall(
        ...
        """
        Prompt determining whether a mask is part of a wall/backsplash (Responses API)
        ...
        instructions = (
            "You are an expert in indoor design and plane fitting.\n\n"
                ...
                "content": [
                    {
                        ...
                        "text": (
                            "### Task Description ###\n"
                            "I will provide an image showing a scene, and a mask where only the target wall or backsplash "
                            "is shown in its original color, while all other objects are masked as black.\n"
                            "Your task is to distinguish if the mask is part of a wall or backsplash. If yes, return y; If no, return n.\n\n"
                            "### Special Requirements ###\n"
                            "Pay attention to the following:\n"
                            "1. Pixels with their original color are pixels belonging to the mask, while black pixels are outside the mask.\n"
                            "2. It is fine if the mask contains pixels belonging to another wall or other objects, but if more than half "
                            "of the mask contains pixels of other objects, please return n.\n"
                            "3. The mask does not need to include a whole wall/backsplash. As long as it includes a part of a wall/backsplash "
                            "without including a significant part of other objects, please return y.\n\n"
                            "### Example Outputs ###\n"
                    ...
                    {
                        "type": "input_image",
                        "image_url": f"data:image/png;base64,{original_img_base64}",
                        "detail": "high"
                    },
                    {
                        "type": "input_text",
                        "text": (
                            "The above image shows a scene in the real world.\n"
                            "The following image shows a possible mask for a wall/backsplash in the image. "
                            "Please determine if it is part of a wall/backsplash following given instructions."
                        )
                    },
                    {
                        "type": "input_image",
                        "image_url": f"data:image/png;base64,{cand_img_base64}",
                        "detail": "high"
                    }
                ]
            }
    ...
    def payload_select_object_from_list(self, img_path, obj_list, bbox_img_path, nonproject_obj_img_path):
        ...
        instructions = (
            "You are an expert in object captioning.\n\n"
            "### Task Overview ###\n"
            "The user will show you a scene, a target object in the scene highlighted by a bounding box, "
            "and the mask of the target object in the scene where only the target object is shown in its original color and all other objects are masked as black pixels.\n\n"
        ...
        input_messages = [
            {
                ...
                "content": [
                    ...
                    {
                        "type": "input_image",
                        "image_url": f"data:image/png;base64,{base64_target_obj_mask_image}",
                        "detail": "high"
                    },
                    {
                        "type": "input_text",
                    ...
                    {
                        "type": "input_image",
                        "image_url": f"data:image/png;base64,{base64_target_obj_bbox_image}",
                        "detail": "high"
                    },
                    {
                        "type": "input_text",
    ...
    def payload_mount_type(
        ...
        instructions = (
            "You are an expert in indoor design, orientation estimation, and plane fitting.\n\n"
            "### Task Description ###\n"
        ...
        input_content = [
            ...
            {
                "type": "input_image",
                "image_url": f"data:image/png;base64,{bbox_base64}",
                "detail": "high"
            },
            {
                "type": "input_text",
        ...
        for i in range(len(cand_imgs_base64)):
            ...
            input_content.append({
                "type": "input_image",
                "image_url": f"data:image/png;base64,{cand_imgs_base64[i]}",
                "detail": "high"
            })
        ...
        input_content.append({
            "type": "input_text",
            "text": (
                f"To help you better decide if the target object ({caption}) is installed/fixed on one or multiple wall(s)/backsplash(es), "
        ...
        input_content.append({
            "type": "input_image",
            "image_url": f"data:image/png;base64,{obj_and_wall_mask_base64}",
            "detail": "high"
        })
    ...
    def payload_align_wall(
        ...
        instructions = (
            "You are an expert in indoor design, orientation estimation, distance estimation, and plane fitting.\n"
        ...
        input_content = [
            ...
            {
                "type": "input_image",
                "image_url": f"data:image/png;base64,{obj_mask_base64}",
                "detail": "high"
            },
            {
                "type": "input_text",
        ...
        input_content.append({
            "type": "input_text",
            "text": (
                f"To help you better decide if the target object ({caption}) is aligned with and makes a physical contact with one or multiple wall(s)/backsplash(es), "
                "I also provide the following image shows all wall(s)/backsplash(es) and the target object in their original color, "
                "while all other objects and planes are masked as black."
            )
        })
    ...
    def payload_count_drawer_door(self, caption, bbox_img_path, nonproject_obj_img_path):
        """
        Generates custom prompt payload for counting doors and drawers (Responses API)
        ...
        instructions = (
            "You are an expert in indoor design, and articulated furniture design.\n"
            "Your task is to count the number of doors (revolute) and drawers (prismatic) of an object/furniture."
            "I will give you an image showing a scene in our everyday life where an object in the same scene highlighted by a bounding box, and the mask of the object. "
            "Please tell me how many doors (those with revolute joint, that can rotate around an axis) and drawers (those with prismatic joint, that can slide along a direction) does the target object have.\n"
    ...
    def payload_nearest_neighbor(
        ...
        instructions = (
            ...
            "I will show you an image of a scene. "
        ...
        input_content = [
            {
                "type": "input_text",
                "text": f"The following image shows a scene in the real world."
            },
            {
                "type": "input_image",
                "image_url": f"data:image/png;base64,{original_img_base64}",
            ...
            {
                "type": "input_text",
                "text": f"The following image shows the same scene with the target object ({caption}) highlighted by a bounding box."
            },
            {
                "type": "input_image",
                "image_url": f"data:image/png;base64,{bbox_base64}",
            ...
            {
                "type": "input_image",
                "image_url": f"data:image/png;base64,{mask_obj_img}",
                "detail": "high"
            },
            {
                "type": "input_text",
                "text": f"The following are a list of assets you can choose to represent the {caption}. Please choose the asset with highest geometric similarity to the target object ({caption}), i.e., choosing the best digital twin for the target object."
            }
        ]
        for i, cand_base64 in enumerate(cand_imgs_base64):
            input_content.append({
                "type": "input_text",
                "text": f"image {i + 1}:"
            })
            input_content.append({
                "type": "input_image",
                "image_url": f"data:image/png;base64,{cand_base64}",
                "detail": "high"
            })
    ...
    def payload_articulated_nearest_neighbor(
        ...
        input_content = [
            ...
            {
                "type": "input_text",
                "text": f"The following are a list of assets you can choose to represent the {caption}. Please choose the asset with highest geometric similarity to the target object ({caption}), i.e., choosing the best digital twin for the target object. Do not include explanation."
            }
        ]
        for i, cand_base64 in enumerate(cand_imgs_base64):
            input_content.append({
    ...
    def payload_nearest_neighbor_pose(
        ...
        instructions = (
            "You are expert in orientation estimation and feature matching.\n\n"
            "### Task Description ###\n"
        ...
        input_content = [
            ...
            {
                "type": "input_text",
                "text": f"The following image shows the target object ({caption}) bounded by a bounding box."
            },
            {
                "type": "input_image",
                "image_url": f"data:image/png;base64,{bbox_base64}",
    ...
    def payload_task_object_extraction_and_spatial_reasoning(
        ...
        """
        Generates the prompt payload for extracting necessary objects and their initial 3D spatial reasoning
        based on a given task, scene, and annotated image.

        Args:
            annotated_image_path (str): Absolute path to the annotated image (will be base64 encoded).
            scene_objects (list[str]): List of objects currently in the scene.
            goal_task (str): The task to be performed in the simulation.

        Returns:
            dict: Prompt payload for the API call.
        """
        # Getting the base64 string
        annotated_img_base64 = self.encode_image(annotated_image_path)

        instructions = (
            "You are a professional simulation designer.\n"
...
### Example 4 ###
Example Input:
Task: Put the book on the desk
Scene image: <image>
Scene objects: ["desk_0", "chair_0", "lamp_0"]

Example Output:
The book should be placed flat on the desk surface, ensuring it’s fully supported and not overlapping with other objects.
{{
    "scenario_0": {{
        "objects": {{
            "book": {{
...
### Example 5 ###
Example Input:
Task: Put the cup on the shelf above the sink
Scene image: <image>
Scene objects: ["sink_0", "shelf_0", "cabinet_0"]

Example Output:
The cup should be placed securely on the shelf surface, positioned toward the front edge so it’s visible and reachable.
{{
    "scenario_0": {{
        "objects": {{
            "cup": {{
                "parent_object": "shelf_0",
                "placement": "above"
            }}
        }}
    }}
...
### Example 6 ###
Example Input:
Task: Take the sandwich on the plate and move it to the table
Scene image: <image>
Scene objects: ["plate_0", "table_0", "chair_0"]

Example Output:
The sandwich currently rests on the plate. To complete the task, it should be transferred onto the table’s surface.
{{
    "scenario_0": {{
        "objects": {{
            "sandwich": {{
...
### Example 7 ###
Example Input:
Task: Put the keys on the counter near the refrigerator
Scene image: <image>
Scene objects: ["counter_0", "refrigerator_0"]

Example Output:
The keys should be placed on the flat surface of the counter, close to the refrigerator’s right side.
{{
    "scenario_0": {{
        "objects": {{
            "keys": {{
                "parent_object": "counter_0",
                "placement": "above"
            }}
        }}
    }}
...
"""
        ...
        input_content = [
            ...
            {
                "type": "input_image",
                "image_url": f"data:image/png;base64,{annotated_img_base64}", # <-- Flattened structure
                "detail": "high"
            }
        ]
...
#                 {
#                     "role": "system",
#                     "content": contentm uses a lighter to light a decorative candle to enhance the table ambiance.\n" + \
