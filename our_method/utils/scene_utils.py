...
def align_model_pose(
    ...
    # 2D bounding box를 이용한 z-axis 보정 (fine-tune)
...
def align_model_quat(cam_pos, cam_quat, tilt_angle=0.0, obj_z_angle=0.0):
    """
    카메라 기준 회전 정보만으로 오브젝트 쿼터니언 계산 (point cloud 없이)

    Args:
        cam_pos (np.ndarray): (3,) 카메라 위치 (x, y, z)
        cam_quat (np.ndarray): (4,) 카메라 회전 quaternion (x, y, z, w)
        tilt_angle (float): x축 기준 회전 각도 (rad)
        obj_z_angle (float): z축 기준 회전 각도 (rad)

    Returns:
        np.ndarray: 최종 오브젝트 회전 quaternion (x, y, z, w)
    """
    # 카메라에서 OG 프레임으로 변환 (180도 회전)
    cam_local_tf = T.pose2mat(([0, 0, 0], T.euler2quat([np.pi, 0, 0])))
    cam_global_tf = T.pose2mat((cam_pos, cam_quat))

    # 추가적으로 기울어진 각도와 z축 회전 적용
    tilt_mat = T.euler2mat([tilt_angle, 0, 0])
    z_rot_mat = T.euler2mat([0, 0, -obj_z_angle])

    # 최종 회전 행렬
    final_rot_mat = cam_global_tf[:3, :3] @ cam_local_tf[:3, :3] @ tilt_mat.T @ z_rot_mat.T

    # 회전 행렬 → 쿼터니언
    obj_quat = T.mat2quat(final_rot_mat)
...
def compute_object_z_offset_non_articulated(target_obj_name, sorted_obj_bbox_info, verbose=False):
    '''
    Computes object below @target_obj_name and the necessary z-offset required to place the target object on top of
    below object's bounding box.
    ...
    for check_obj_name in reversed(obj_names_to_check):  # Start from larger z value
        ...
        cand_obj_frame_to_world = sorted_obj_bbox_info[check_obj_name]["desired_frame_to_world"]
        # intersection_area_threshold = 0.3 if sorted_obj_bbox_info[check_obj_name]["articulated"] and not sorted_obj_bbox_info[target_obj_name]["articulated"] else 0.5
        intersection_area_threshold = 0.3
        cand_vertex_desired_frame = np.concatenate([cand_obj_bbox_bottom_desired_frame, cand_obj_bbox_top_desired_frame], axis=0)
        assert len(cand_vertex_desired_frame) == 8
        cand_vertex_world_frame = trimesh.transformations.transform_points(cand_vertex_desired_frame, cand_obj_frame_to_world)
        cand_vertex_world_frame_xy = [pt[:-1] for pt in cand_vertex_world_frame]
...
def compute_obj_bbox_info(obj):
    ...
    "lower"	바운딩 박스의 최하단 (AABB 기준)
    "center"	바운딩 박스의 중심 (AABB 기준)
    "upper"	바운딩 박스의 최상단 (AABB 기준)
    "bbox_bottom_in_desired_frame"	객체의 로컬 프레임에서 바운딩 박스 하단 4개 좌표
    "bbox_top_in_desired_frame"	객체의 로컬 프레임에서 바운딩 박스 상단 4개 좌표
    "desired_frame_to_world"	객체의 로컬 좌표를 세계 좌표로 변환하는 4x4 행렬
    """
    obj_pos, obj_quat = obj.get_position_orientation()
