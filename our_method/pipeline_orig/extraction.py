...
from our_method.utils.processing_utils import (
    create_polygon_from_vertices, NumpyTorchEncoder, filter_large_masks,
    unprocess_depth_linear, process_depth_linear, compute_point_cloud_from_depth, annotate, mask_intersection_area, mask_area,
...
class RealWorldExtractor:
    ...
    def __call__(
        ...
        Args:
            ...
            depth_image (None or np.ndarray): If specified, the depth image corresponding to the input RGB image.
                Should be a 2D array (H, W) with depth values in meters. If not specified, it will be automatically
                estimated using DepthAnythingV2
    ...
    # Calculate resize scale so the longer edge becomes 1600
    ...
    # Update input_path to the resized image filename
    ...
    # Save resized image
        ...
        if self.verbose:
            log.info(f"Input image ({raw_width}, {raw_height}) resized to ({new_width}, {new_height})")
            log.info(f"Saved resized image to {input_path}")
    ...
    # Create GPT instance
        ...
        if self.verbose:
            log.info(f"Extracting real-world info from image {input_path}...")
        ...
        if captions is None:
            ...
            if self.verbose:
                log.info(f"Detected raw object captions: {captions}")
    ...
    # Convert remaining backsplash/wall masks to color masks
        ...
        if self.verbose:
            ...
            log.debug("Sub-Step 2. Run DepthAnythingV2 to extract synthetic depth map")

        if camera_intrinsics_matrix is None:
            if self.verbose:
                log.info(f"No K intrinsics matrix given, estimating...")
            ...
            # Apply resize scale to camera intrinsics
            scaled_intrinsics = camera_intrinsics_matrix.copy()
            ...
            camera_intrinsics_matrix = scaled_intrinsics
            
            if self.verbose:
                log.info(f"Camera intrinsics scaled by factor {scale_factor:.4f}, K matrix:\n{camera_intrinsics_matrix}")


        if self.verbose:
            log.info(f"Estimating depth map...")

        depth_path = f"{save_dir}/step_1_depth.png"
        depth_limits = np.array([0, depth_max_limit])
        
        # Load or estimate depth image
        if input_depth_path is not None and os.path.exists(input_depth_path):
            # Load depth image from file
            depth = np.load(input_depth_path)
            output_shape = (new_height, new_width)
            depth_image = process_depth_linear(depth=depth, in_limits=depth_limits, out_shape=output_shape)
            Image.fromarray(depth_image).save(depth_path)
            if self.verbose:
                log.info(f"Loaded depth image from {input_depth_path}, shape: {depth_image.shape}, dtype: {depth_image.dtype}")
        else:
            ...
            depth_image = depth_estimator.estimate_depth_linear(input_path=input_path, output_path=depth_path, depth_limits=depth_limits)
        ...
        if self.verbose:
            log.debug("Sub-Step 4. Use GSAMv2 to extract bounding boxes for inferred objects")
        ...
        if self.verbose:
            log.info(f"Detected parsed unique object categories: {detected_objs}")

        # Compute masks and bounding boxes using GSAM
        image_source, image = load_image(input_path)

        if self.verbose:
            log.info(f"Predicting input image bounding boxes using GroundingDINO...")

        boxes, logits, phrases = self.fm.gsam.predict_boxes(image, ". ".join(detected_objs.keys()))

        if self.verbose:
            log.debug("Sub-Step 5. Post-process masks to remove noise and differentiate instances within the same mask")
        ...
        if self.verbose:
            log.info(f"Pruning redundant bounding boxes...")
        ...
        for idx in sorted(idxs_to_remove, reverse=True):
            ...
            del masks[idx]
            if self.verbose:
                log.info(f"remove {idx} {phrases[idx]}")
        ...
        if self.verbose:
            log.info(f"Remaining phrases after pruning: {phrases}")
            log.info(f"Object names: {names}")
        ...
        if self.verbose:
            log.info(f"Initial phrases: {phrases}")
            log.info(f"Final phrases after recaption: {phrases_recaptioned}")
        ...
        if self.verbose:
            log.info(f"Saved Step 1 Output information to {step_1_output_path}")
        ...
        if self.verbose:
            log.info(f"Saved extracted information to {detected_categories_path}")
