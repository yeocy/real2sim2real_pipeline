...
# from omnigibson.objects import URDFObject  # 파일 상단 import 추가
...
#############################################
    ...
    @staticmethod
    def load_cousin_scene(scene_info, visual_only=False, robot=False):
        """
        ...
        # Franka 로봇 생성
        robot = FrankaPanda(
            name="franka_robot",          # 씬 내에서 고유한 이름
            # relative_prim_path="/World/franka",  # Prim path (선택적)
            reset_joint_pos=[0.0, -0.785398, 0.0, -2.356194, 0.0, 2.570796, 0.785398, 0.04, 0.04],
            fixed_base=True,              # 로봇 베이스가 고정되는지 여부
            grasping_mode="physical",     # 그립 방식: physical / assisted / sticky
            end_effector="gripper",       # 사용할 end effector (예: gripper)
            visual_only=False,      # 시각적 전용으로 로봇을 로드할지 여부
        )
        ...
        #     th.tensor([0.0, 0.0, 0.0], dtype=th.float),  # 원하는 위치
        #     th.tensor([0, 0, 0, 1], dtype=th.float)      # 원하는 쿼터니언
        # )
        # robot.set_joint_positions(
        #     th.tensor([0.0, -0.785398, 0.0, -2.356194, 0.0, 1.570796, 0.785398], dtype=th.float)  # 원하는 조인트 위치
        # )
