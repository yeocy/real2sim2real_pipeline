...
class RealSceneGenerator:
    ...
    def __call__(
        ...
        if self.verbose:
            log.info(f"Generating simulated scenes given output {step_2_output_path}...")
        ...
        for scene_count in range(n_scenes):
            log.info(f"[Scene {scene_count + 1} / {n_scenes}]")
            ...
            for obj_idx, (obj_name, obj_cousin_idx) in enumerate(cousin_idxs.items()):
                if self.verbose:
                    log.info(f"[Scene {scene_count + 1} / {n_scenes}] [Object {obj_idx + 1} / {n_objects}] generating...")
            ...
            if self.verbose:
                log.info(f"[Scene {scene_count + 1} / {n_scenes}] refining scene graph...")
            ...
            if resolve_collision:
                if self.verbose:
                    log.info(f"[Scene {scene_count + 1} / {n_scenes}] depenetrating collisions...")
                ...
                if self.verbose:
                    log.info(f"[Scene {scene_count + 1} / {n_scenes}] skip depenetrating collisions.")

            # Put objects down
            if self.verbose:
                log.info(f"[Scene {scene_count + 1} / {n_scenes}] placing all objects down...")
