...
from our_method.utils.dataset_utils import (
    get_all_dataset_categories, get_all_articulated_categories,
...
class DigitalCousinMatcher:
    ...
    def __init__(self, feature_matcher, verbose=False):
        """
        Args:
            feature_matcher (FeatureMatcher): Feature matcher instance for segmenting and matching features.
    ...
    def __call__(
        ...
        for obj_indexes, categories in zip(
            ...
            if len(obj_phrases) > 0:
                if self.verbose:
                    log.info(f"Computing top-{top_k_categories} for phrases: {obj_phrases}")
        ...
        for instance_idx, (box, logit, phrase, name) in enumerate(
            ...
            # Replace category spaces with underscores for naming
            category = phrase.replace(" ", "_")

            if self.verbose:
                log.info("-----------------")
                log.info(f"[Object {instance_idx + 1} / {n_instances}] Finding for object {name}, category: {category}...")
            ...
            for i in range(n_digital_cousins):
                log.info(f"Selecting digital cousin {i+1} / {n_digital_cousins}")
                # Reselect candidates
                if i % n_cousins_reselect_cand == 0 or i >= n_cousins_link_count_threshold:
                    ...
                    if is_articulated:
                        # Only select candidates with valid number of doors/drawers for articulated objects
                        if self.verbose:
                            log.info(f"Articulated object being matched, filtering candidate models to valid number of doors / drawers")
                        ...
                        if i < n_cousins_link_count_threshold:
                            ...
                            for candidate_img_fpath in raw_all_cand_img_fpaths:
                                # Path: <ASSET_PATH>/objects/<category>/snapshot/<category>_<model>.png
                    ...
                    if self.verbose:
                        log.info(f"Selecting Top-{top_k_models} nearest models using {self.fm.encoder_name}...")
                    ...
                    if len(candidate_imgs) < max_candidates:
                        ...
                        for batch_idx in range(num_batches):
                            ...
                            candidate_imgs_temp = candidate_imgs[batch_idx * max_candidates:(batch_idx + 1) * max_candidates]
                        ...
                        model_results = self.fm.find_nearest_neighbor_candidates(
                            ...
                            phrases=[phrase],
                            obj_masks=obj_masks,
                            save_prefix=f"{name}_iter{num_batches}",
                            remove_background=remove_background,
                        )
                ...
                # Select the nearest model using GPT or DINOv2
                if gpt_select_cousins:
                    # Select the nearest model via GPT
                    if self.verbose:
                        log.info(f"Selecting cousin #{i} final model using GPT...")
                    if is_articulated:
                    ...
                    # Query GPT
                    gpt_text_response = gpt(payload=nn_selection_payload, verbose=self.verbose)
                    if gpt_text_response is None:
                        log.info(gpt_text_response)
                    ...
                    if self.verbose:
                        log.info(f"Selecting cousin #{i} final model using DINOv2...")
                    nn_model_index = 0
                ...
                if self.verbose:
                    log.info(f"Selecting Top-{top_k_poses} nearest poses using {self.fm.encoder_name}...")
                ...
                # For articulated objects, only search among valid frontal face orientations
                ...
                if self.verbose:
                    log.info(f"Selecting cousin #{i} final pose using GPT...")
